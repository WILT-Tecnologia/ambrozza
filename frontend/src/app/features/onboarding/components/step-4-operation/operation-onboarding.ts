    ...
    <div [formGroup]="form()" class="space-y-6">
      ...
      <div class="space-y-3.5 pt-2">
        <label [class]="getLabelClass()">
          ...
          <div>
            ...
            <span class="text-xs text-stone-500 mt-0.5 block">
              Quero enviar os bolos e doces fresquinhos direto na casa dos clientes.
            </span>
          </div>
        </label>
        ...
        <label [class]="getLabelClass()">
          ...
          <div>
            <span class="block font-bold text-sm text-stone-900">
              Retirada na Confeitaria / Balcão
            </span>

            <span class="text-xs text-stone-500 mt-0.5 block">
              Quero permitir que venham retirar encomendas ou comprar pronta-entrega no local.
            </span>
          </div>
        </label>
        ...
        @if (isNoneSelected()) {
          <span class="text-[11px] text-rose-600 font-semibold mt-1.5 block">
            Selecione pelo menos uma forma de entrega ou retirada.
          </span>
        }
      </div>
      ...
      >
        ...
        <span>
          ...
          poderão ser ajustados com calma logo após a abertura.
        </span>
      </div>
    </div>
  `,
...
export class Step4OperationComponent {
  ...
  isNoneSelected(): boolean {
    const delivery = this.form().get('allowDelivery')?.value;
    const pickup = this.form().get('allowPickup')?.value;
    return !delivery && !pickup && (this.form().touched || this.form().dirty);
  }

  getLabelClass(): string {
    return getLabelClasses(this.isNoneSelected());
  }
}
